...
print('''
 ...
 node{2,3}, those nodes will appear to NodeMonitor to be offline.  Our starting condition
 is that node1 and node2 are active, and node3 is inactive.  After 15s, node2
 should be deactivated, a new primary node will be chosen, and and our DummyAgent will start 
 printing these events.
''')
def testNodeMonitor1(nm):
    nm.start()
    print("Waiting for 20 secs, watch for output from DummyAgent")
    time.sleep(20)
    nm.stop()
    time.sleep(1)
